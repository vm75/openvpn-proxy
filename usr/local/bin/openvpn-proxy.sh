...
apps_setup() {
    if [ ! -f /.apps_initialized ] ; then
        log "Onetime setup for apps"
        if [ -f /data/apps.sh ] ; then
            /data/apps.sh setup
        fi

        touch /.apps_initialized
    fi
}
...
function main() {
    ...
    while [ ${shutting_down} == "no" ] ; do
        ...
        if [ ! -f ${VAR_DIR}/ovpn.conf ] || [ ! -f ${VAR_DIR}/vpn.auth ] ; then
            log "VPN config not found. Retrying openvpn connection after ${RETRY_INTERVAL} seconds..."
            sleep ${RETRY_INTERVAL};
            continue
        fi
